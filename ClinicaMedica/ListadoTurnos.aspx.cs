...
namespace ClinicaMedica
{
    public partial class ListadoTurnos : System.Web.UI.Page
    {
        ...
        protected void HabilitacionDeAcceso()
        {
            if (Session["UsuarioActivo"] != null)
            {
                for (int i = gvTurnos.Columns.Count - 1; i >= 0; i--)
                {
                    if (gvTurnos.Columns[i] is CommandField)
                    {
                        gvTurnos.Columns.RemoveAt(i);
                    }
                }

                CommandField ShowButton = new CommandField();
                ...
                {          
                    ...
                    gvTurnos.DataSource = gestionTablas.ObtenerTablaTurnos(ConfiguracionTurno);
                    gvTurnos.Columns.Add(ShowButton);
                    gvTurnos.DataBind();
                }
            }
            else
        ...
        protected void gvTurnos_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            ...
            gvTurnos.DataSource = gestionTablas.ObtenerTablaTurnos(filtro, Convert.ToInt32(ddlEstados.SelectedValue));
            gvTurnos.DataBind();
        }
        ...
        protected void btnBuscar_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(txtBuscador.Text))
            {
                ...
                DataTable tablaFiltrada = gestionTablas.ObtenerTablaTurnos(PorDni, Convert.ToInt32(ddlEstados.SelectedValue));
